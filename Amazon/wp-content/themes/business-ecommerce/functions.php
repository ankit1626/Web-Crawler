...
if ( isset( $_GET['never_show_info'] ) ) {
		update_option('never_show_info', 'yes');
} else {
	$business_ecommerce_info = get_option('never_show_info', 'no');
	if ($business_ecommerce_info != 'yes'){
		add_action( 'admin_notices', 'business_ecommerce_admin_notice_info' );
	}
}

define ('business_ecommerce_pro_theme_url', 'https://wpfreetheme.space/product/business-ecommerce/');
